...
function display_usage {
	echo 'Usage : vsh [-start port (directory)] [-stop port (directory)] [-list destination port] [-browse destination port archive_name] [-extract destination port archive_name]'
	exit 0
}
...
function check_config {
	local list='nmap netcat-openbsd'
	local package
	local result
	local status
